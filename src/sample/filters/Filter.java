...
public class Filter {
    ...
    private int getNewPixel(int[][] extendMatrix, int[][] stampingKernel, int i, int j) {
        ...
        return new Color(
                getValidColorValue((int) Math.round(sumR)),
                getValidColorValue((int) Math.round(sumG)),
                getValidColorValue((int) Math.round(sumB))).getRGB();
    }

    private int[][] getStampingKernel() {
        return new int[][]{{-1, -1, -1}, {-1, 10, -1}, {-1, -1, -1}};
    }
